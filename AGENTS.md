...
- No local URLs, debug flags, demo credentials, or stale `Tech Staff Evaluation Platform` branding remain where they would leak into production

## Known Repo-Specific Notes

- `PROJECT_GUIDE.md` describes the product correctly; any remaining `Tech Staff Evaluation Platform` naming outside explicit historical notes should be treated as stale and updated
